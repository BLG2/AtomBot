...
module.exports = {
  help: {
    name: "reset",
    aliases: ["rst"],
    description: `Reset everyones invites`,
    examples: [`reset`],
  ...
  async run(bot, message, args) {
    ...
    const saved = await bot.inviteServices.ResetAllMemberInvitesAsync(message.guild.id);
    switch (saved?.acknowledged ?? false) {
      case true:
        let onembed = new moduleHolder.Modules.discord.EmbedBuilder()
          .setDescription(`${moduleHolder.Modules.emojie.succes} Resetted everyones invites`)
